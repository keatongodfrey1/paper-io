...
- On the start screen, set up your game: **difficulty** (Easy/Normal/Hard/Extreme), **board size**, **game speed**, **win condition**, and **number of bots** (0-30). Then press Start
...
- The bar up top is the live **standings**. Win by reaching an adjustable **target %** (set in 5% steps), or by holding the **most territory** when an adjustable **timer** (30-second steps) runs out
...
**Phase 1 so far:** a settings screen (difficulty, board size, speed, adjustable
target-%/timed win, 0-30 bots), randomized starts, a countdown, territory stealing,
kill-to-absorb, connected-territory rules, live standings, pause, and bots that respawn
during play. Still to come: a little polish and the optional "ghost racing" bonus, then
Phase 2 (couch multiplayer) and Phase 3 (make-your-own sandbox). The full plan lives in the
design doc under `~/.gstack/projects/PaperIOClone/`.
...
the bot respawn interval (`SPAWN_INTERVAL`). To use a sound your kids record, follow the note
